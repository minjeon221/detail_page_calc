$(document).ready(function(){
    ...
    $("select#crush option:eq(0), select#gram option:eq(0)").prop("selected", true) //최초 로딩시 - 필수항목 - 을 표시한다.
    
    function calc_price(){
        //각 항목에 대한 추가
        $total = 0; //값이 추가되는 부분을 막아야 함
        for(i=0; i<$each_total_price.length;i++){
            $total += $each_total_price[i]; //최종 배열데이터 내부의 모든 값을 더한다.
        }
        if($total == 0){ //전체 합산이 0이라면 
            $(".det_total_price").hide(); 
            $("select option").prop("selected", false); //모두 선택 해제 
            $("select#crush option:eq(0), select#gram option:eq(0)").prop("selected", true) //선택을 -필수 항목-으로 변경한다.
        }else{
    ...
    $(document).on("click", ".item_del", function(){
        ...
        $each_total_price.splice($del_index, 1); //해당하는 인덱스 번호의 데이터를 삭제하고 나머지는 저장
    ...
    //조건 1번 셀렉트 박스가 선택이 된 상태 이후 2번 셀렉트 박스가 선택이 되었을 때 change 이벤트를 걸어 세부 항목인 .my_item을 ul아래의 마지막 자식에 추가
    $(".form_crush select").change(function(){
        $(".form_gram select").removeAttr("disabled");  
        $("select#gram option:eq(0)").prop("selected", true)
    ...
    $(".form_gram select").change(function(){
        ...
        var $opt_02_val = parseFloat($(this).val()); //parseFloat($(".form_gram select").val())
        ...
        console.log($each_total_price); // 옵션값 포함한 가격을 배열데이터로 저장(변동)
    ...
    $(document).on("click", ".minus", function(){
        ...
        if($amount[$index] != 1){
            $amount[$index]--; //수량 감소
            console.log($amount);
            $(this).siblings("input").val($amount[$index]); //감소수량 표기
    ...
    $(document).on("click", ".plus", function(){
        ...
        $(this).siblings("input").val($amount[$index]); //증가수량 표기
        $each_total_price[$index] = $each_calc_price[$index] * $amount[$index] //최종 배열 데이터 = 기본값 * 수량
        console.log($each_total_price);
        console.log($each_calc_price);
